...
cpt = torch.load(inputPath, map_location=torch.device('cpu'))
...
test_rnd = torch.rand(1, 192, 200).float()
...
mlmodel = ct.converters.convert(traced_model,
    ...
    inputs=[
        ...
        ct.TensorType(name='rnd', shape=test_rnd.shape, dtype=np.float32),
        ],
